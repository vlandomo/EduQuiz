  ...
  function startQuiz(questions) {
    ...
    function loadQuestion() {
      ...
      currentQuestion.options.forEach((option, index) => {
        ...
        label.innerHTML = `<input class="radio_btn" type="radio" name="answer" value="${option}" ${
    ...
    function checkAnswer(selectedOption) {
      ...
      if (!isCorrect) {
        ...
        nextQuestion(); // الانتقال الي السؤال التالي اذا كان الجواب صحيحا
      }
    }
      ...
      if (wrongAnswers > 0) {
        ...
        wrongQuestions.forEach((wrong) => {
          resultContainer.innerHTML += `
                        <div>
                         <table class = "showresult_table">
        <tr>${wrong.question}</tr>
        <tr>
            <th>Your Answer</th>
            <td>${wrong.userAnswer}</td>
        </tr>
        <tr>
        ...
        <td colspan = "2">${wrong.explanation}</td>
    </table>
                    `;
        });
